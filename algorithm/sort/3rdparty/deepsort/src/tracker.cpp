...
void tracker::update(const DETECTIONS &detections,int detection_idx)
{   
    ...
    {
        this->_initiate_track(detections[detection_idx],detection_idx);
    }
    //#ifdef MY_inner_DEBUG
...
{   
    ...
    for (it = tracks.begin(); it != tracks.end();)
    {
        if ((*it).is_match==3) {
            (*it).hits += 1;
            (*it).time_since_update = 0;
            if ((*it).state == Track::TrackState::Tentative && (*it).hits >= (*it)._n_init)
            {
                (*it).state = Track::TrackState::Confirmed;
            }
            
        }else if((*it).is_match==0){
            (*it).mark_missed();
        }
        (*it).is_match = 0;
        if ((*it).is_deleted())
    ...
    FEATURESS dummy_feat = FEATURESS::Zero(1, 512);
    dummy_feat(0, 0) = 1.0f;
    for (Track &track : tracks)
    {
...
// 2. 直接塞入固定死的假特征，彻底不读 track.features，避开所有崩溃风险
        tid_features.push_back(std::make_pair(track.track_id, dummy_feat));
    }
    // printf("active_targets size = %d\n", active_targets.size());
...
void tracker::_initiate_track(const DETECTION_ROW &detection,int index)
{
    ...
    this->tracks.push_back(Track(mean, covariance, this->_next_idx, this->n_init,
                                 this->max_age, detection.feature, detection.cls, detection.conf,index));
    _next_idx += 1;
}
