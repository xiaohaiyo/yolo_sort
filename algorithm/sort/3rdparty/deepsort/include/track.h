...
public:
    ...
    Track(KAL_MEAN& mean, KAL_COVA& covariance, int track_id,
          int n_init, int max_age, const FEATURE& feature, float cls, float conf,int is_match);
    ...
    KAL_MEAN mean;
    KAL_COVA covariance;
    float cls;       // 新增：保存目标的类别索引
    float conf;      // 新增：保存最新的检测置信度
    int hits;
    int age;
    int _n_init;
    int _max_age;
    TrackState state;
    //是否更新匹配上,0:未匹配,1:可见匹配，2红外匹配
    int is_match;
private:
